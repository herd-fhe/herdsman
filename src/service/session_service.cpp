...
{
	auto user_sessions = sessions_.equal_range(user_id);

	auto session_name_equals = [&session_name](const auto& key_value) {
		return key_value.second.name == session_name;
	};

	auto session_iter = std::find_if(user_sessions.first, user_sessions.second, session_name_equals);

	return session_iter != user_sessions.second;
}
...
void SessionService::destroy_session_by_name(uint64_t user_id, const std::string& session_name)
{
	...
	const auto user_sessions = sessions_.equal_range(user_id);
	const auto to_remove_iter = std::find_if(
			user_sessions.first, user_sessions.second,
...
{
	const auto user_sessions = sessions_.equal_range(user_id);
	sessions_.erase(user_sessions.first, user_sessions.second);
}
...
std::vector<SessionService::Session> SessionService::sessions_by_user(uint64_t user_id) const
{
	...
	sessions.reserve(static_cast<std::size_t>(std::distance(user_sessions.first, user_sessions.second)));

	std::transform(user_sessions.first, user_sessions.second, std::back_inserter(sessions), [](const auto& key_value) {return key_value.second;});
